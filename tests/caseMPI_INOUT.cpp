...
int main(int argc, char*argv[]){
    ...
    ff_endpoint g1_tcp("127.0.0.1", 49000);
    g1_tcp.groupName = "G1";

    ff_endpoint g4_tcp("38.242.220.197", 65000);
    g1_tcp.groupName = "G1";
    ...
    if (myrank == 0){
        ...
        ReceiverManager *recMaster = new ReceiverManager({new ReceiverPluginTCP(g1_tcp, 1)}, {{0, 0}});
        ...
        rt[std::make_pair(g4_tcp.groupName, ChannelType::FWD)] = std::vector<int>({0});
        ...
        SenderManager* sendMaster = new SenderManager({{{g4_tcp.groupName}, new SenderPluginTCP({{ChannelType::FWD, g4_tcp}}, "G4")}}, &rt);
        gFarm.add_collector(new ff_dMPsender(sendMaster));

        gFarm.cleanup_emitter();
        Snk *snk = new Snk(total_task, check);

        gFarm.add_workers({new WrapperINOUT(snk, 1, true)});
